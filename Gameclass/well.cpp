...
bool Well::tetriminoFit(Tetrimino tetrimino) {
	...
	for (int i = 0; i < TETRIMINO_GRID_SIZE; i++) {
		for (int j = 0; j < TETRIMINO_GRID_SIZE; j++) {
			...
			if (currentTetrimino[i][j] == 1 && ((i + currentTetriminoLocation.row < 0 || j + currentTetriminoLocation.col < 0) || 
				(i + currentTetriminoLocation.row > 23 || j + currentTetriminoLocation.col > 7))) {
				doesItFit = false;
			}
		}
	}
